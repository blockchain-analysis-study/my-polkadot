...
///
/*
构造parachain-agreement(平行链 协议)实例
*/
	...
	<N::BuildTableRouter as IntoFuture>::Future: Send + 'static,
{
	...
	如果尚未启动，则在父哈希顶部启动一个parachain协议进程。

	此外，这将触发向新区块的 (职责候选名单) 广播数据。
	*/
		...
		-> Result<Arc<AttestationTracker>, Error>
	{
		...
		/*
		如果我们知道它们，计算出 父 [区块的??]候选人。
		这将允许我们根据需要将传出消息传播给其他对等方。
